...
class CSPRNG:
    ...
    def reset_entropy_count(self):
        self.entropy_count = 0

    def mix_pool_bytes(self, random_event: str) -> None:
        """Mix new random events into the existing entropy pool."""
        self.update_entropy_count()  # Track entropy introduced.

        random_event_numerical = str(
            ord(random_event)
        )  # Represent key press numerically.

        new_entropy_bytes = hashlib.blake2s(
            random_event_numerical.encode("utf-8")
        ).digest()

        self.entropy_pool.update(new_entropy_bytes)
        self.entropy_pool_in_bytes = self.entropy_pool.digest()
